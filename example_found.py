...
class stream_of_ticks(list):
    ...
    def as_pdDataFrame(self):
        # if no data -> do a blank tick
        if len(self) == 0:
            return tick(datetime.datetime.now()).as_pandas_row()

        pd_row_list = [x.as_pandas_row() for x in self]
        pandas_df = pd.concat(pd_row_list)
...
class tick(object):
    ...
    def as_pandas_row(self):
        ...
        attributes = ['bid_size','bid_price', 'ask_size', 'ask_price', 'last_trade_size', 'last_trade_price']
